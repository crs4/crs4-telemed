...
class AccessToken(OAuthView, Mixin):
    ...
    * :attr:`invalidate_refresh_token`
    ...
    def get_pincode_grant(self, request, data, client):
        """
        Return a user associated with this request or an error dict.
    ...
    def pincode(self, request, data, client):
        """
        Handle ``grant_type=password`` requests as defined in :rfc:`4.3`.
        """
        import logging
        logging.error('In PINCODE')
        data = self.get_pincode_grant(request, data, client)
        user = data.get('user')
        scope = data.get('scope')
    ...
    def post(self, request):
        ...
        logging.error('Try with handle: %s and grant_type %s' % (handler, grant_type))
